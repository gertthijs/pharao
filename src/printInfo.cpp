...
printInfo(const std::string& option) 
{
   ...
   if (option == "r" || option == "reference")
   {
      ...
      std::cerr << " only the database will be processed without screening." << std::endl;
      std::cerr << std::endl;
  ...
  if (option == "noAlign")
  {
      ...
      std::cerr << std::endl;
      std::cerr << " Flag to indicate when no alignment should be computed. In this case" << std::endl;
      std::cerr << " only the pharmacophore points will be computed" << std::endl;
      std::cerr << std::endl;
      valid = true;
  }
